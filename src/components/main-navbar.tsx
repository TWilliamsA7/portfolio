    ...
    <header className={headerClasses}>
      <nav className="flex items-center justify-between w-full max-w-7xl px-4 md:px-8">
        ...
        <Link href="/" className="text-2xl font-bold tracking-tight text-foreground hover:text-primary transition-colors">
          <span className="text-primary">Tai Williams</span>
        </Link>
                    ...
                    <div className="flex flex-col space-y-4 pt-6">
                        {navLinks.map((link) => (
                            <Link
                                key={link.href}
                                href={link.href}
                                className="text-lg font-semibold text-foreground hover:text-primary transition-colors py-2 border-b border-border"
                            >
                                {link.title}
                            </Link>
                        ))}
